...
if(isset($_POST['option_val'])) {
	...
	$sql_exist="SELECT * FROM tbl_option where option_name='".$m."' AND status= 1" ;
	$oResult_exist=$oBasic->SqlQuery($sql_exist);
	if($oResult_exist->num_rows>0){
		 $data['html']=1;
		  echo json_encode($data);
	}else{
		$sql="INSERT INTO  tbl_option (option_name,status) 
				  VALUES ('$m','1')";
