<?php
	...
	if (isset($_POST['btnSave']))
	{
		...
		{
			$sql="INSERT INTO product (Heading, Photo, Message, Summary, SortID, CreatedBy, CreateDate) VALUES ('$Heading', '$pic', '$Message', '$Summary', '$SortID', '$CreatedBy', '$CreateDate')";
			$oResult=$oBasic->SqlQuery($sql);
			//print_r($oResult);
			echo "<script>alert('Insert Perform Successfully');</script>";
			echo "<script>window.location='?Basic=OP'</script>";
		}
		elseif($_POST['btnSave']=='Update')
		{
			$sql="UPDATE product SET Heading='$Heading', Photo='$pic', Message='$Message', Summary='$Summary', SortID='$SortID' WHERE ID = '$ID'";
			$oResult=$oBasic->SqlQuery($sql);
